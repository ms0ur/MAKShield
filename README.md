...
> ⚠️ **Firefox для Android поддерживает расширения начиная с версии 120+ (Fenix)**

1. На Android-устройстве откройте **Firefox** (не Firefox Lite/Focus)
2. Перейдите в **Настройки** → **О Firefox** → нажмите на номер версии 5 раз для включения режима разработчика
3. Перейдите в **Настройки** → **Пользовательская коллекция дополнений**
4. Введите ID вашего аккаунта Mozilla и название коллекции, куда вы загрузили MAKShield
5. Перезапустите Firefox — расширение появится в разделе Дополнения

**Альтернатива (через ADB):**
1. Включите **отладку по USB** на устройстве
2. Подключите к ПК, откройте `about:debugging` в **десктопном** Firefox
3. Выберите устройство → **«Загрузить временное дополнение»**
4. Выберите `manifest.json` с ПК
